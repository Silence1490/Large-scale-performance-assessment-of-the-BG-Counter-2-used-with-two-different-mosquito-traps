...
akwtranslated2_plot<- as.data.frame(
  rbind(
    cbind(
...
# final lm is identified with corresponding counter data 
cor_days<-final_lm%>%
...
mea<- as.vector(
  rbind(
    ...
    paste0(
      round(
        mean_trans_day$estimate[1],1
        ),
        " (CI:",round(
          mean_trans_day$conf.int[1],1),
        " - ",
      round(
        mean_trans_day$conf.int[2],1),
      ")"),
    paste0(
      round(
          mean_capt_day$estimate[1],1),
          " (CI:",round(
            mean_capt_day$conf.int[1],1),
      " - ",
      round(
        mean_capt_day$conf.int[2],1),
      ")"),
    paste0(
      round(
        mean_trans_suc$estimate[1],1),
          " (CI:",round(
            mean_trans_suc$conf.int[1],1),
      " - ",
      round(
        mean_trans_suc$conf.int[2],1),
      ")"
      )
    )
  )
...
accuracy<-ifelse(
  dfan_tt$Identified>dfan_tt$Counted,
  ((dfan_tt$Counted/dfan_tt$Identified)*100),
  ((dfan_tt$Identified/dfan_tt$Counted)*100)
  )
accuracy<-ifelse(
  dfan_tt$Counted==0 & dfan_tt$Identified==0,
  100,
  accuracy
 )

dfan_tt<-as.data.frame(
  cbind(
    dfan_tt, accuracy
    )
  )
...
daily_accuracy<- as.data.frame(
  cbind(
    min=min(dfan_tt$accuracy),
...
```{r, include=F}
###Linear regression predicting captured by counted including the corresponding plot
...
location_suff_lm<-dfan_tt_suff %>%
  group_by(Location, Trap) %>%
  summarise(
    rsq=rsq(log1p(Identified),log1p(Counted)),
            pv=p.value(log1p(Identified),log1p(Counted)),
            est=est(log1p(Identified),log1p(Counted)),
            ac= mean(accuracy), 
            "Trap days"= mean(N),
            "Captured mosquitoes"=sum(Identified),
            " Mean Captured mosquitoes"=mean(Identified)
    )%>%
  ungroup()
...
tot_names<-c("Location", "Trap", "lm: R-squared","lm: p-value", "lm: Estimate",
             "Accuracy (%)","Trap days","Captured mosquitoes","Mean captured mosquitoes",
             "Spearman Correlation", "cor: p-value")
...
tot_tab<-tot_tab %>% mutate_at(
  vars(
    "lm: R-squared", "lm: Estimate",
    "Spearman Correlation", "Mean captured mosquitoes"
    ),funs(round(., 3)))
...
su<-as.vector(
  rbind("Sum","",paste0(round(
        sum(as.numeric(tot_tab$`Trap days`)),1
        )
      ),
   paste0(round(
       sum(as.numeric(tot_tab$`Captured mosquitoes`)),1
       )
     )
                    ,"","","","","","","")
              )
tot_tab2<-rbind(tot_tab,mea,su)
tot_tab2$Location<-recode_factor(
  as.factor(
    tot_tab2$Location),
  "1" = "Hamburg 1", "2" = "Neu Wulmsdorf",
  "3" = "Hamburg 2", "4" = "Varel 1",
  "5" = "Großheide", "6" = "Gronau Landau",
  "7" = "Buchholz", "8" = "Weinheim",
  "9" = "Tübingen", "10" = "Fürth",
...
# Aggregating the calculated accuracies per site and per month and the corresponding plots
marg<-1
small<-35
...
dfan_tt_month<-  dfan_tt%>%pivot_longer(cols=c(
  accuracy, Identified),
  names_to="method",
  values_to="value")
...
dfan_tt_month2<-dfan_tt%>%
  group_by(month)%>%
  summarise(
    method="Identified",mean=mean(Identified), 
    se=sd(Identified)/sqrt(length(Identified)),
    sd=sd(Identified),n= length(month), 
    marg=qt(0.975,df=length(month)-1)*sd(Identified)/sqrt(length(Identified)))%>%
  ungroup()
...
dfan_tt_month3<-dfan_tt%>%
  group_by(month)%>%
  summarise(
    method="Accuracy",mean=mean(accuracy), 
    se=sd(accuracy)/sqrt(length(accuracy)),
    sd=sd(accuracy),n= length(month), 
    marg=qt(0.975,df=length(month)-1)*sd(accuracy)/sqrt(length(month)))%>%
  ungroup()
...
accplt0<-ggplot(bla,aes(x=as.factor(month), y=mean))+
    geom_pointrange(aes(ymin=mean-marg, ymax=mean+marg, color=method),
                    position=position_dodge(0.4), size=1.3)+
    scale_color_manual(values=c("Accuracy"="#E69F00", "Identified"="#56B4E9"),
                      breaks=c("Accuracy", "Identified"), 
                      labels = c("Accuracy (+-SE)", "Captured (+-SE)"))+
...
accplt4<- ggplot(dfan_tt, aes(x=as.factor(size), y=as.numeric(accuracy), fill=size))+
  ...
  geom_boxplot(
    ...
    ylab("Accuracy (%)")+
    stat_summary(
      aes(fill=as.factor(size)),
      show.legend = F,fun.y=mean, geom="point", shape=4, stroke=2, size=4,position=position_dodge(width=0.5))+
    theme_classic()

accplt4<-accplt4+theme(
  legend.title = element_blank(),
...
trap_comp<-subset(
  dfan_tt,
  dfan_tt$Location==1|dfan_tt$Location==2|
  dfan_tt$Location==3|dfan_tt$Location==4|
  dfan_tt$Location==5|dfan_tt$Location==6|
  dfan_tt$Location==7|dfan_tt$Location==8|
...
AIC(ac_ident_trap_glmm1,ac_ident_trap_glmm2)
...
lintrapplot2<-ggplot(data = trap_comp3, aes(y = accuracy/100, x = log1p(Identified), color = Trap)) +
  ...
  theme_classic()+
  scale_color_manual(values=c("PG"="#E69F00", "PR"="#009E73"),breaks=c("PG", "PR"),
                     labels=c("PG" =   expression(bold("CO"[2]*"-Pro-gravid")), 
...
# comparing the the power of the random effect of Location

ac_ident_trap_glmm2<-glmer((accuracy/100)~(Identified)+Trap+(1|Location),data=trap_comp3, family = binomial)
summary(ac_ident_trap_glmm2)
trap_comp4<-trap_comp3
trap_comp3$zero_group <- 1
ac_ident_trap_glmm3<-glmer(
  (accuracy/100)~(Identified)+Trap+(1|zero_group),data=trap_comp3, 
  family = binomial,
...
```{r, include=F}
# does the counter over or underestimate the actual amount of mosquitoes
underestimate<-ifelse(
  trap_comp3$Identified>trap_comp3$Counted,
  "underestimate",
  ifelse(
    trap_comp3$Identified<trap_comp3$Counted,
    "overestimate","exact"
...
catch_rel<- as.data.frame(cbind(
  ID=ident$ID,Species=ident$species,
...
sella_pgpr<-subset(
  selladf,
  selladf$Location==1|selladf$Location==2|
  selladf$Location==3|selladf$Location==4|
  selladf$Location==5|selladf$Location==6|
  selladf$Location==7|selladf$Location==8|
...
# in the following we generate corresponding values 
# for example if we only captured a single sella 7 mosquito in one day
# we generate a row with zero caught sella 1 mosquitoes and vice versa
...
# checking if we are not generating data for trap days we don't have  captured mosquitoes for
...
# calculating the shannon diversity for the different trap versions
...
# comparing amount of captured mosquitoes among the two trap versions
...
```{r, include=F}
#multiple plots for Trap comparison
marg<-1
small<-20
accplt<- ggplot(trap_comp3, aes(x=Trap, y=as.numeric(accuracy), fill=Trap))+
  ...
  "PR" = expression(bold("CO"[2]*"-Pro"))))+
  stat_summary(aes(fill=as.factor(Trap)),show.legend = F,
  fun.y=mean, geom="point", shape=4, stroke=2, size=3,position=position_dodge(width=0.5))+
...
divplt<- ggplot(div_comp2, aes(x=Trap, y=shannon, fill=Trap))+
  ...
  stat_summary(aes(fill=as.factor(Trap)),
               show.legend = F,fun.y=mean, geom="point", shape=4, stroke=2, 
               size=3,position=position_dodge(width=0.5))+
  scale_fill_discrete(labels=c(
    "PG" = expression(bold("CO"[2]*"-Pro-gravid")), 
    "PR" = expression(bold("CO"[2]*"-Pro"))))+
  theme_classic()+
  scale_x_discrete(
    labels=c(
...
Traps_sel<- ggplot(selladf3, aes(y=log1p(as.numeric(number)),x=factor(new_sella, levels=c("1","7")),fill=Trap))+
  ...
  geom_boxplot( position = position_dodge(0.6), width = 0.24, alpha = 0.5)+
  stat_summary(aes(fill=as.factor(Trap)),show.legend = F,fun.y=mean, geom="point",
               shape=4, stroke=2, size=3,position=position_dodge(width=0.6))+
  scale_fill_discrete(labels=c(
    "PG" = expression(bold("CO"[2]*"-Pro-gravid")), 
    "PR" = expression(bold("CO"[2]*"-Pro"))))+
  scale_fill_manual(values = c("#E69F00","#009E73"))+
...
### All Results in order of appearance in the research paper
print("Abstract")
...
# what follows is the reconstruction of the paper knitted as pdf
```
...
```{r fig4,message=F,warning=F, dpi=1000, echo=F ,out.width="100%", fig.cap="\\label{fig4}Trap version comparison with boxplots, half-density distributions and means: A) number of caught mosquitoes, B) captured mosquitoes with either a Sella score of 1 (unfed) or higher (blood-fed), C) Shannon diversity index D) BG-Counter accuracy"}
#multiple plots
marg<-1
small<- 8
accplt<- ggplot(trap_comp3, aes(x=Trap, y=as.numeric(accuracy), fill=Trap))+
  ...
  scale_x_discrete(labels=c(
    ...
    "PR" = expression(bold("CO"[2]*"-Pro"))))+
  stat_summary(aes(fill=as.factor(Trap)),
               show.legend = F,fun.y=mean, geom="point", shape=4, stroke=1.5, 
               size=3,position=position_dodge(width=0.5))+
  scale_fill_discrete(labels=c(
    "PG" = expression(bold("CO"[2]*"-Pro-gravid")), 
    "PR" = expression(bold("CO"[2]*"-Pro"))))+
...
divplt<- ggplot(div_comp2, aes(x=Trap, y=shannon, fill=Trap))+
  ...
  ylab("Shannnon diversity index")+
  stat_summary(aes(fill=as.factor(Trap)),show.legend = F,
               fun.y=mean, geom="point", shape=4, stroke=1.5, 
               size=3,position=position_dodge(width=0.5))+
  scale_fill_discrete(labels=c(
    "PG" = expression(bold("CO"[2]*"-Pro-gravid")), 
    "PR" = expression(bold("CO"[2]*"-Pro"))))+
...
Traps_sel<- ggplot(selladf3, aes(y=log1p(as.numeric(number)),x=factor(new_sella, levels=c("1","7")),fill=Trap))+
  ...
  geom_boxplot(
    ...
    alpha = 0.5)+
  stat_summary(aes(fill=as.factor(Trap)),show.legend = F,
               fun.y=mean, geom="point", shape=4, stroke=1.5, size=3,
